...
public class EaseChatLayout extends RelativeLayout implements IChatLayout, IHandleMessageView, IPopupWindow
    ...
    //发送消息检测
    private OnExpireSendMsgCallback callback;
...
//        }
    ...
    public void sendBigExpressionMessage(String name, String identityCode) {
        if(callback != null && callback.isExpireSendMsg()){
    ...
    public void onTouchItemOutside(View v, int position) {
        Log.e("tag","onTouchItemOutside");
        inputMenu.hideSoftKeyboard();
        inputMenu.showExtendMenu(true);
        inputMenu.showEmojiconMenu(false);
    }
    ...
    public void addExpireSendMsgCallback(OnExpireSendMsgCallback callback) {
        this.callback = callback;
    }

    public interface OnExpireSendMsgCallback {

        boolean isExpireSendMsg();
    }
