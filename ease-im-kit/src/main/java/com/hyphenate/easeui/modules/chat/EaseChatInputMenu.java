...
public class EaseChatInputMenu extends LinearLayout implements IChatInputMenu, EaseChatPrimaryMenuListener, EaseEmojiconMenuListener, EaseChatExtendMenuItemClickListener {
    ...
    private FrameLayout extendMenuContainer, extendMenuEmojiContainer;
    ...
    protected void onFinishInflate() {
        ...
        extendMenuEmojiContainer = findViewById(R.id.extend_menu_emoji_container);
    ...
    private void init() {
        ...
        extendMenuEmojiContainer.setVisibility(GONE);
    }
    ...
    public void showEmojiconMenu(boolean show) {
        if(show) {
            //hideSoftKeyboard();
            //showEmojiconMenu();
            extendMenuEmojiContainer.setVisibility(VISIBLE);
        }else {
            extendMenuEmojiContainer.setVisibility(GONE);
            //showSoftKeyboard();
        }
    ...
    private void showEmojiconMenu() {
        ...
        if(emojiconMenu instanceof View) {
            extendMenuEmojiContainer.setVisibility(VISIBLE);
            extendMenuEmojiContainer.removeAllViews();
            extendMenuEmojiContainer.addView((View) emojiconMenu);
            emojiconMenu.setEmojiconMenuListener(this);
        }
        if(emojiconMenu instanceof Fragment && getContext() instanceof AppCompatActivity) {
            extendMenuEmojiContainer.setVisibility(VISIBLE);
            FragmentManager manager = ((AppCompatActivity) getContext()).getSupportFragmentManager();
            manager.beginTransaction().replace(R.id.extend_menu_emoji_container, (Fragment) emojiconMenu).commitAllowingStateLoss();
            emojiconMenu.setEmojiconMenuListener(this);
        }
    }
