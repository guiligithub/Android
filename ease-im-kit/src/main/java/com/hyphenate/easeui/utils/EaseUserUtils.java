...
//        userProvider = EaseUI.getInstance().getUserProfileProvider();
    ...
    public static void setUserAvatar(Context context, String username, ImageView imageView) {
        ...
        if(TextUtils.equals(username, EMClient.getInstance().getCurrentUser())) {
            EaseUser mineUser = getUserInfo(EMClient.getInstance().getCurrentUser());
            if(mineUser != null){
                if(!TextUtils.isEmpty(mineUser.getAvatar())) {
                    Glide.with(context).load(mineUser.getAvatar()).apply(new RequestOptions()
                            .diskCacheStrategy(DiskCacheStrategy.ALL)
                            .skipMemoryCache(true)).into(imageView);
                }else {
                    //己方头像
                    if (mineUser.getGender() == 0) {
                        Glide.with(context).load(R.drawable.icon_woman_default).into(imageView);
                    } else {
                        Glide.with(context).load(R.drawable.icon_man_default).into(imageView);
                    }
                }
            }
        } else {
            ...
            if (user != null) {
                if (!TextUtils.isEmpty(user.getAvatar())) {
                    try {
                        ...
                        //use default avatar
                        RequestOptions options = RequestOptions.placeholderOf(R.drawable.ease_default_avatar)
                                .diskCacheStrategy(DiskCacheStrategy.ALL)
                    ...
                    EaseUser mineUser = getUserInfo(EMClient.getInstance().getCurrentUser());
                    //对方头像
                    if (mineUser != null && mineUser.getGender() == 0) {
                        Glide.with(context).load(R.drawable.icon_man_default).into(imageView);
                    } else {
                        Glide.with(context).load(R.drawable.icon_woman_default).into(imageView);
                    }
