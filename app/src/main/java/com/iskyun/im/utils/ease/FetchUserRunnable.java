package com.iskyun.im.utils.ease;
    ...
    private void warpEMUserInfo(Map<String, EMUserInfo> userInfos){
        ...
        while(it_user.hasNext()) {
            ...
            if (userInfo != null) {
                ...
                //LogUtils.e("userInfo:"+ GsonUtils.toJson(userInfo));
                ...
                info.setHeadImage(userInfo.getAvatarUrl());
                EaseLiveDataBus.get().with(EaseCallKitUtils.UPDATE_USERINFO).postValue(info);
            }
        }

        //更新本地数据库信息
        ImHelper.getInstance().updateUserList(userEntities);
        //if(refreshContact) {
            //通知UI刷新列表
            EaseEvent event = EaseEvent.create(Constant.CONTACT_UPDATE, EaseEvent.TYPE.CONTACT);
            event.message = userInfos.keySet().toString();
            //发送联系人更新事件
            LiveDataBus.get().with(Constant.CONTACT_UPDATE).postValue(event);
        //}
